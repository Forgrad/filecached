 ...
 *share文件 返回1
 *                 */
int private_or_share(char filename[])
{
    return 1;
}
...
{ 
    ...
    file->flag = private_or_share(filename);
    if(file->flag == 1)/*share_file*/
    {
        struct share_file *share_file_node=hash_entry(node,struct share_file,hnode);/*获取相应的存放共享数据信息的结构*/
        file->file_info = share_file_node;
    }else{            /*private_file*/
        return NULL;
    }
...
{
    struct share_file *node=(struct share_file *)file->file_info;/*获取相应的存放共享数据信息的结构*/
...
char* dmf_gets(char* buf, size_t size, dmf_file* file)
{
    ...
    struct share_file *node=(struct share_file *)file->file_info;
...
{
    struct share_file *s_node=(struct share_file *)file->file_info;
